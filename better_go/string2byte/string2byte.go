...
/*
	    https://github.com/thinkeridea/go-extend/blob/main/exstrings/convert.go
...
*/
func unsafeToBytes1(s string) []byte {
	xx := unsafe.Pointer(
		&struct {
			string
			Len int
...
func unsafeToBytes2(s string) []byte {
	aa := unsafe.Pointer(&s)
	x := (*[2]uintptr)(aa)
	xx := *(*reflect.StringHeader)(unsafe.Pointer(x))
	fmt.Println(xx)
	b := [3]uintptr{x[0], x[1], x[1]}
	return *(*[]byte)(unsafe.Pointer(&b))
}

func directToBytes(string2 string) []byte {
	return []byte(string2)
}
